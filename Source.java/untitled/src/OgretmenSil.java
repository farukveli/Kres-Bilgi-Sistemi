...
public class OgretmenSil {

	Connection con;
	OgretmenSil(Connection con) {
		this.con = con;
		JFrame frame = new JFrame("Ogretmen Silme Sayfasi");
		...
		try {
			...
			while (rs.next()) {
				model.addElement(rs.getString("teachId") + " " + rs.getString("firstName") + " " + rs.getString("lastName") + " " + rs.getString("address") + " " + rs.getString("phone"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		buton1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (list.getSelectedIndex() == -1) {
					...
					try {
						PreparedStatement statement = con.prepareStatement("DELETE FROM teacher WHERE teachid=?");
